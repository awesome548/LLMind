...
| mind-elixir integration | `simple-mindmap.tsx` | How we wrap the library: init-once + `refresh(data)` on model change (`skipRefreshRef` breaks echo loops); selection sync both ways (map click → `onSelect`; external `activeNodeId`/`activeTopic` → `selectNode`, id preferred — M-E5); structural edits stream back via the `operation` bus event; wheel-zoom/`mouseSelectionButton: 2` mirror the shared canvas grammar (`view-interactions.ts`). Library API reference: `node_modules/mind-elixir/readme.md` (the vendored `Mind-elixir.md` copy was deleted in the 2026-07-03 consolidation) |
...
| Session I/O | `src/lib/session-io.ts` + navigator Save/Load | Full exploration state as versioned JSON (capture, crash recovery, sharing); `restoreSession` store action. **The load path is the trust boundary (M-E3):** `parseSessionFile` type-checks every slice and resets any malformed one to its default (surfacing a `toast.warning`), so a corrupt file degrades instead of white-screening. A render-level `src/app/error.tsx` boundary catches any throw that still slips through — "Reload view" recovers from the persisted store (M-E4) |
| Usage counters | store `usage` + `trackUsage` | Feature-usage instrumentation; included in session files |
| Study mode | `src/lib/study-bundle.ts` + navigator flask button + store `participantId` | **M-E12:** a `?p=<id>` URL param (or a facilitator prompt) tags the session; the flask button exports ONE `llmind-study-bundle` file — the restorable session (event log + usage + reflections inside), the markdown record, and the computed stats — named `llmind-bundle-<participant>-<date>.json`. **Pre-warm** before a participant arrives by opening the Structure→Schema view (triggers annotation) and letting the rationale query settle, with the LLM stack up — the cold-cache latency (§5.6 weakness 4) then never lands on the participant |
